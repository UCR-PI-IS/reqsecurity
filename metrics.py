...
def calculate_metrics(
    ...
    metric["TP"] = len(set(oracle[use_case][sentence].keys()) & set(results))
    metric["FP"] = len(set(results) - set(oracle[use_case][sentence].keys()))
    metric["FN"] = len(set(oracle[use_case][sentence].keys()) - set(results))
    return metric

def calculate_score(df: pd.DataFrame) -> pd.DataFrame:
    df["Relevance"] = df["TP"] / (df["TP"] + df["FP"])
    df["Coverage"] = df["TP"] / (df["TP"] + df["FN"])
    df["Efficiency"] = df["TP"] / (df["execution_time"] / 60.0)
...
def compute_metrics_by_similitude(groups, oralce):
    requirements = []
    for group in groups:
        for run in groups[group]:
            for result in run["treatment"]["results"]:
    ...
    groups = df.groupby(
        ['id', 'use_case', 'sentence', 'group', 'objective', 'pattern']
    ...
    for _, row in groups.iterrows():
        ...
        if id not in oralce[row['use_case']][row['sentence']]:
            metrics.extend([metric.copy() for _ in range(len(generated_requirements))])
            continue
        oracle_requirement = oralce[row['use_case']][row['sentence']][id]
